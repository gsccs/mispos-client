...
import com.allinpay.mis.logic.init.Init4pxp2p;
...
public class SnapManage {
	...
	public static boolean createSnap(Map<String, Table> tableMap) {
		...
		try {
			...
			StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.currentSnapDirBackFail") + e1.toString(), LogLevel.ERROR);
			e1.printStackTrace();
			return isSuccess;
		}
			...
			if (!"".equals(snapName.trim())) {
				...
				if (!snapBefore.exists()) {
					try {
						...
						StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.ifLastSnapExistFail") + e.toString(), LogLevel.ERROR);
						e.printStackTrace();
					}
				} else {
					// 删除原快照，将当前快照变成原快照
				...
				try {
					...
					StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.newSnapFileFail") + e.toString(), LogLevel.ERROR);
					e.printStackTrace();
		...
		for (String snapName : tableMap.keySet()) {
			if (!"".equals(snapName.trim())) {
				...
				if (snapMD5Before == null) {
					StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.beforeSnapMd5Fail") + snapBefore.getName(), LogLevel.ERROR);
					return false;
				}
				if (snapMD5Now == null) {
					StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.nowSnapMd5Fail") + snapNow.getName(), LogLevel.ERROR);
					return false;
				}
				if (snapMD5Before.equals(snapMD5Now)) {
					...
					try {
						...
						StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.lineByLineDiffFail") + e.toString(), LogLevel.ERROR);
						e.printStackTrace();
						return false;
					} finally {
	...
	public static void recoverSnapBak() {
		...
		try {
			...
			StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.restoreSnapBackFail") + e.toString(), LogLevel.ERROR);
			e.printStackTrace();
		}
	}
	...
	public static boolean diffCsvLineByLine(File snapBefore, File snapNow) throws Exception {
		...
		if (!Arrays.equals(headerBefore, headerNow)) {
			StatusLog.setStatusDetail(snapName + PropUtil.getProperty("ds.logic.tableChanged"), LogLevel.ERROR);
			return false;
		}

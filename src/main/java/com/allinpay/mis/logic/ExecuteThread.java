...
import com.allinpay.mis.logic.bean.Table;
...
public class ExecuteThread extends Thread implements ExecuteThreadInterface {
    ...
    public boolean testLink() {
        StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.testLinking"), LogLevel.INFO);
        ...
        try {
            ...
            if (connSQLServer == null) {
                isLinkedPass = false;
                StatusLog.setStatusDetail("SQLServer " + PropUtil.getProperty("ds.logic.testLinkFail"), LogLevel.ERROR);
            }

            if (connMySQL == null) {
                isLinkedPass = false;
                StatusLog.setStatusDetail("connMySQL " + PropUtil.getProperty("ds.logic.testLinkFail"), LogLevel.ERROR);
            }

            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.testLinkFinish"), LogLevel.INFO);

        } catch (Exception e) {

            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.testLinkFail") + e.toString(), LogLevel.ERROR);
            isLinkedPass = false;
        } finally {
            if (dbSQLServer != null) {
                try {
    ...
    public boolean analyseConfigFile() {
        StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.startAnalyse"), LogLevel.INFO);
        ...
        for (File file : tableFieldFiles) {
            ...
            if (!fileName.endsWith(".sql")) {
                ...
                try {
                    ...
                    StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.AnalyseFail") + e.toString(), LogLevel.ERROR);
                    e.printStackTrace();
                }
                progressValue++;
                StatusPanel.progressCurrent.setValue(progressValue);
        ...
        try {
            ...
            isAnalyseSuccess = false;
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.AnalyseTriggerFail") + e.toString(), LogLevel.ERROR);
            e.printStackTrace();
        } catch (IOException e) {
            isAnalyseSuccess = false;
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.AnalyseTriggerFail") + e.toString(), LogLevel.ERROR);
        ...
        if (isAnalyseSuccess) {
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.finishAnalyse"), LogLevel.INFO);
        }
     ...
     */
    public void backUp() {
        StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.startBackUp"), LogLevel.INFO);
        ...
        try {
            ...
            logger.error(PropUtil.getProperty("ds.ui.database.to.err.decode") + e.toString());
            e.printStackTrace();
        }
        BackupManage.exportDatabase(ConstantsTools.CONFIGER.getHostTo(), user, password,
                ConstantsTools.CONFIGER.getNameTo());

        StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.finishBackUp"), LogLevel.INFO);
    }
    ...
    public boolean newSnap() {
        StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.startNewSnap"), LogLevel.INFO);
        boolean isSuccess = true;

        isSuccess = SnapManage.createSnap(originalTablesMap);
        if (isSuccess) {
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.finishNewSnap"), LogLevel.INFO);
        }
    ...
    public boolean diffSnap() {
        StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.startDiffSnap"), LogLevel.INFO);
        boolean isSuccess = true;

        isSuccess = SnapManage.diffSnap(originalTablesMap);

        if (isSuccess) {
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.finishDiffSnap"), LogLevel.INFO);
        } else {
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.diffSnapFail"), LogLevel.ERROR);
        }
    ...
    public boolean executeSQL() {
        StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.startRunSql"), LogLevel.INFO);
        ...
        if (sqlList.size() != 0) {
                ...
                try {
                    ...
                    if ("true".equals(ConstantsTools.CONFIGER.getStrictMode())) {
                        if (affectedRecords == totalSqls) {
                            ...
                            isSuccess = false;
                            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.runSqlFail"), LogLevel.ERROR);
                            return false;
                        }
                    } else {
                        DbUtilMySQL.getInstance().getConnection().commit();
                    ...
                    StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.runSqlFinish"), LogLevel.INFO);
                    StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.syncFinish"), LogLevel.INFO);
                    return isSuccess;
                } catch (Exception e) {
                    try {
                        DbUtilMySQL.getInstance().getConnection().rollback();
                    ...
                    isSuccess = false;
                    StatusLog.setStatusDetail("!!!" + sqlForLog + PropUtil.getProperty("ds.logic.currentSqlFail") + e.toString(), LogLevel.ERROR);
            ...
            StatusPanel.progressCurrent.setValue(1);
            StatusLog.setLastTime(Utils.getCurrentTime());
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.runSqlFinish") + totalSqls, LogLevel.INFO);
            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.syncFinish"), LogLevel.INFO);
        }
                    ...
                    if (isDiffSuccess) {
                        ...
                        if (isExecuteSuccess) {
                            ...
                            if (!StatusPanel.buttonStartSchedule.isEnabled()) {
                                StatusLog.setStatus(PropUtil.getProperty("ds.logic.runScheduleing"));
                            } else {
                                StatusLog.setStatus(PropUtil.getProperty("ds.logic.manuSyncFinish"));
                            }
                            StatusLog.setStatusDetail(PropUtil.getProperty("ds.logic.currentManuSyncFinish"), LogLevel.INFO);
                            // 设置持续时间
